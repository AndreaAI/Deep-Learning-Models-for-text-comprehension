...
class MemNN_KV(object):
    ...
    def __init__(self, batch_size, vocab_size,
        ...
        #                                initializer=tf.contrib.layers.xavier_initializer(False))
        self.TK = tf.get_variable('TK', shape=[self._memory_key_size, self.reader_feature_size],
                                  initializer=tf.contrib.layers.xavier_initializer(False))
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            self.W = tf.concat([nil_word_slot, tf.get_variable('W', shape=[vocab_size-1, embedding_size],
                                                                  initializer=tf.contrib.layers.xavier_initializer(False))],0)
    ...
    def _build_inputs(self):
        with tf.name_scope("input"):
            ...
            self._memory_key = tf.placeholder(tf.int32, [None, self._memory_key_size, self._story_size], name='memory_key')       
